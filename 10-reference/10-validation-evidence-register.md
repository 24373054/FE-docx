...
|SIM-FORM-GOVERNANCE-001-V1|其余十份模板第一次项目事实演练|Git `28d75d9ac112fcb8fedd3222eb45a8703f79aecb`|2026-07-30T02:45:00+08:00至03:10:00+08:00|records_filled_governance_unapproved|14对象完整性；十份模板首次填写；未发生事项显式阻断|不支持真实受案、人员授权、监管答复、管理评审运行、CAPA关闭或模板批准|
...
|Git依赖|`28d75d9ac112fcb8fedd3222eb45a8703f79aecb`|
|对象数|14|
|root_hash|`82c2d12c2f13941bf55e90188c87003997f576cc69a783ec9574d8f7e70b4a2e`|
|状态|`records_filled_governance_unapproved`|
|证据位置|[`SIM-FORM-GOVERNANCE-001-V1`](../validation-evidence/public/SIM-FORM-GOVERNANCE-001-V1/)|

### 9.2 实际执行

- 用内部研发预审填写FRM-001，并明确拒绝真实案件；
- 用PowerShell/.NET复算环境填写FRM-005，但保持工具未验证；
- 用BlockCypher交易数反例填写FRM-006；
- 用创世输出浏览器地址映射填写FRM-007，并拒绝自然人/控制归属；
- 用既有内部模拟报告填写FRM-011；
- 用错误hex在冻结前被拦截的事件填写FRM-012，但不宣称CAPA关闭；
- 用跨包依赖Schema缺口填写FRM-013；
- 用“公司人员未指定”填写FRM-014，授权结果为none；
- 用已准备未提交的主管机关问题包填写FRM-015；
- 用管理决策包预演FRM-016议程，但不宣称已开会或有决议。
...
|---|---|
|manifest Schema|passed|
|对象存在、字节数和SHA-256|14/14 passed|
|包内父对象引用|passed|
|hashes.sha256与root_hash|passed|
|十份模板首次填完|10/10|
|公司具名复核|0/10|
|模板升级|blocked；每份只有一次演练|

共同发现是通用“草稿/待复核/已批准/已关闭”无法覆盖不同记录的业务生命周期。十份模板已修订为0.2.0候选，但仍保持`模板草案（待实测）`。
...
- 已完成一次14对象治理记录包和其余十份模板第一次演练；所有未发生的组织事实均保持阻断；
- 没有任何区块链专项方法达到V1；
- 没有任何工具版本被验证；
- 没有任何人员获得方法执行授权；
- 没有任何真实案件试点；
...
|版本|日期|变更|状态|
|---|---|---|---|
|0.1.6|2026-07-30|登记其余十份模板第一次实填、14对象治理记录包和未发生事项状态分层|研究底稿|
